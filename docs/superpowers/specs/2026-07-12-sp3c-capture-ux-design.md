...
- **视频编码 = H.264/AVC**(2026-07-13 真机实测:CameraX 1.4 无法强制 HEVC,此 ROM 无 HEVC
  录制档,始终协商到 c2.mtk.avc.encoder;设备虽有 c2.mtk.hevc.encoder 但在 CameraX 够不着处)。
  **HEVC 另立独立子项目**(Camera2+MediaCodec 直驱硬件 HEVC 编码器,省容量/续航,用户决定推迟)。
  音频 AAC/M4A 128kbps;照片 JPEG;录制实测 1920×1080 16:9
