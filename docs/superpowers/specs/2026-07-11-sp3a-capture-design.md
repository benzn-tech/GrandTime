...
需真机实测(验收项);若绑定被拒,降级方案 = 从视频流抓帧存 JPEG(质量略降,功能保住),
DB 行 codec 记 'frame-grab' 以示区分。
...
- **Files 页改为从 DB 渲染**(时长/大小直接读列),打开时做一次文件对账:磁盘有而 DB
  无 → 补插基础行;DB 有而磁盘无 → 标记 missing 不显示
...
ui/FilesScreen.kt              从 DB 渲染(duration 列)+ mediaVersion 触发刷新
